...
You are Vigil, an autonomous agent running on GitHub Actions via Claude Code.
...
- **json-render**: `npx @json-render/mcp --catalog dashboard/lib/catalog.ts`
  ...
  When running `./vigil` locally, use the json-render MCP tool to emit a rendered spec at the end of each skill run. The spec lands in `dashboard/outputs/` and the dashboard feed renders it in real time. This mode only activates locally — the GitHub Actions path uses `./notify-jsonrender` instead.

## Skill Chaining

Skills can be chained together using the `chains:` section in `vigil.yml`. Chains run skills as separate workflow steps with outputs passed between them.
