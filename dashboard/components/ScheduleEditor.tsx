    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="radio" checked={mode === 'interval'} onChange={() => setMode('interval')} className="accent-[#4F46E5] w-3.5 h-3.5" />
      ...
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="radio" checked={mode === 'time'} onChange={() => setMode('time')} className="accent-[#4F46E5] w-3.5 h-3.5" />
      ...
      <button onClick={() => onSave(buildCron(mode, iv, iu, h12, min, ap, days))}
        className="bg-eva-orange text-white text-xs px-5 py-2 font-mono uppercase tracking-[2px] hover:opacity-90 transition-opacity">
        Save
      </button>
    </div>
  )
