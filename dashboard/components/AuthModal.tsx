...
export function AuthModal({ loading, onClose, onAuth }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white border-2 border-[rgba(255,255,255,0.10)] w-full max-w-sm mx-4 p-[var(--space-lg)] shadow-2xl">
        ...
        <button onClick={() => onAuth(authKey.trim())} disabled={!authKey.trim() || loading} className="w-full bg-eva-orange text-white text-sm py-3 font-mono uppercase tracking-[2px] hover:opacity-90 transition-opacity disabled:opacity-50">{loading ? '...' : 'Save'}</button>
      </div>
    </div>
  )
}
