...
Vigil ships with 50+ skills. Each skill is a self-contained markdown instruction file in `skills/<name>/SKILL.md`. Enable any skill in `vigil.yml` and it runs on schedule.

Install any skill into your own agent:
```bash
./add-skill besley1600/vigil <skill-name>
```
...
| `self-review` | Weekly audit of what Vigil did, what failed, and what to improve | Daily 6 PM UTC |
...
./add-skill besley1600/vigil article

# Install multiple skills
./add-skill besley1600/vigil article digest heartbeat

# Install everything
./add-skill besley1600/vigil --all

# Browse available skills
./add-skill besley1600/vigil --list
```

Installed skills land in `skills/` and appear in `vigil.yml` as disabled. Flip `enabled: true` to activate.
