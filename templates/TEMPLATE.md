...
That copies `templates/crypto-tracker/SKILL.md` → `skills/my-token-watcher/SKILL.md`, replaces every `[REPLACE: TOKEN_SYMBOL]` etc. with your value, and registers a disabled entry in `vigil.yml` so you can enable it from the dashboard or CLI when you're ready.
