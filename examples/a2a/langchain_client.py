"""
LangChain ↔ Vigil A2A example.

Wraps an Vigil skill as a LangChain Tool so any LangChain agent can call it.
...
def call_aeon(skill_id: str, var: str = "") -> str:
    ...
    if "error" in submit:
        raise RuntimeError(f"Vigil rejected task: {submit['error']}")
