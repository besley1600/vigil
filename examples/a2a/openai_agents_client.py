"""
OpenAI Agents SDK ↔ Vigil A2A example.

Registers `vigil-token-report` as a function tool the Agent can call when
...
def _call_aeon(skill_id: str, var: str) -> str:
    ...
    for _ in range(120):
        ...
        if state in ("failed", "canceled"):
            raise RuntimeError(f"Vigil {skill_id} {state}: {result['status']}")
