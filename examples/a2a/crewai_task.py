"""
CrewAI ↔ Vigil A2A example.

Wraps `vigil-pr-review` as a CrewAI BaseTool so a Crew agent can review
...
def _call_vigil(skill_id: str, var: str) -> str:
    ...
    for _ in range(120):
        ...
        if status["status"]["state"] in ("failed", "canceled"):
            raise RuntimeError(f"Vigil {skill_id} failed: {status['status']}")
    raise TimeoutError(f"Vigil {skill_id} timed out")


class VigilPRReviewTool(BaseTool):
    name: str = "vigil_pr_review"
    description: str = (
        "Review all open pull requests on a GitHub repo via Vigil. "
    ...
    def _run(self, repo: str) -> str:
        return _call_vigil("vigil-pr-review", repo)


reviewer = Agent(
    role="Senior PR reviewer",
    goal="Triage open PRs on the target repo and recommend which to merge first.",
    backstory="A senior engineer who delegates the heavy reading to Vigil, then "
              "writes the prioritised summary for the team standup.",
    tools=[VigilPRReviewTool()],
    verbose=True,
    allow_delegation=False,
)

if __name__ == "__main__":
    repo = sys.argv[1] if len(sys.argv) > 1 else "besley1600/vigil"
    review_task = Task(
        description=f"Use the vigil_pr_review tool on {repo}, then write a 5-bullet "
