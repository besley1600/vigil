"""
AutoGen ↔ Vigil A2A example.

Registers `vigil-deep-research` as a function tool that an AutoGen
AssistantAgent can call inside a multi-agent conversation. The user agent
asks the assistant to research a topic; the assistant calls Vigil and
returns the report.
...
def call_aeon_skill(skill_id: str, var: str = "") -> str:
    ...
    for _ in range(120):
        ...
        if status["status"]["state"] in ("failed", "canceled"):
            raise RuntimeError(f"Vigil skill {skill_id} failed: {status['status']}")
