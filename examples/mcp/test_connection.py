#!/usr/bin/env python3
"""
Sanity-check the Vigil MCP server end-to-end.

What it does:
  1. Spawns `node mcp-server/dist/index.js` as a stdio MCP server.
  2. Sends `tools/list` and prints every vigil-* tool that is registered.
  3. Calls one tool (default: `vigil-cost-report`, fast and offline-safe)
...
def repo_root() -> Path:
    """Walk up from this file until we find the Vigil repo root."""
    here = Path(__file__).resolve()
    for candidate in (here, *here.parents):
        if (candidate / "skills.json").exists() and (candidate / "mcp-server").is_dir():
            return candidate
    raise SystemExit(
        "Could not locate Vigil repo root (no skills.json + mcp-server/ above this file)."
    )
...
if __name__ == "__main__":
    tool = sys.argv[1] if len(sys.argv) > 1 else "vigil-cost-report"
