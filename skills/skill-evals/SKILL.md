...
- `vigil.yml` — registered skills, enabled flags, cron schedules.
...
Do not re-implement coverage detection in prose. If the script fails, fall back to the in-memory check (compare `evals.json` keys to `vigil.yml` enabled skills) and mark `eval-audit=fail` in the source-status footer.
...
## Coverage Gaps (enabled in vigil.yml, missing from evals.json)
...
All inputs are local files (`evals.json`, `vigil.yml`, `memory/*`, `articles/*`, `scripts/eval-audit`). No outbound HTTP — no fallback needed. `./scripts/eval-audit` is a local bash script and uses `jq`; if jq is missing (rare on GH Actions ubuntu runners), the script will exit non-zero — mark `eval-audit=fail` in the source-status footer and continue with the in-memory coverage check.
