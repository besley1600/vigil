...
If skills.json is missing from upstream (vanishingly unlikely on an Vigil-shaped parent) → log `FORK_SKILL_GAP_NO_UPSTREAM_MANIFEST`, exit (no notify).
...
- **Bot owner allowlist:** `dependabot[bot]`, `github-actions[bot]`, `vigilframework[bot]` — skip from cohort rendering and from totals (they're never running the agent themselves; counting them inflates "active fork" numbers).
