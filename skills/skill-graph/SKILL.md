...
Today is ${today}. Generate a navigable, decision-ready Mermaid map of all Vigil skills. Skip notify and PR when nothing changed.
...
**Explicit edges:**
- `vigil.yml` → per-skill `enabled`, `schedule`, `var`, `model`; `chains:` blocks (`steps:`, `consume:`, `parallel:`); `reactive:` blocks (`trigger:`, `on:`, `when:`)
...
- Enabled state comes from `vigil.yml` only — never infer from cron-state or recent runs (a skill can be enabled but not yet run).
