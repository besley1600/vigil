...
> **${var}** — Optional. Pass `dry-run` to skip the notification (article still writes, log still appends). Pass a fork repo slug (e.g. `someuser/vigil`) to read remote `vigil.yml` + `skills.json` from that fork instead of the local working tree (useful for surveying the fleet ahead of a v4 announcement). Empty = audit the local fork.
...
Reads:
- `vigil.yml` — enabled skills, model overrides, chain definitions, schedule strings, reactive triggers, gateway block, channels block.
...
|---------|----------------|--------------------|
| `chains:` runner interface | `vigil.yml` | Step format (`parallel:` / `consume:` keys, `on_error` semantics) |
| `reactive:` trigger conditions | `vigil.yml` | Condition vocabulary (`consecutive_failures`, `success_rate`, `last_status`) |
| Chain runner output passing | `.outputs/*.md`, `chain-runner.yml` | Path layout, file shape |
| Schedule syntax (`workflow_dispatch`, `reactive`, cron) | `vigil.yml` | Naming of pseudo-schedules; cron escapes |
...
|-------|-------|------------------------|
| `vigil.yml` | direct read | `gh api repos/${TARGET}/contents/vigil.yml --jq .content \| base64 -d` |
...
If `vigil.yml` is unreadable, log `V4_READINESS_NO_CONFIG` and exit with no notification (the fork is not initialized; nothing to check).

### 3. Compute the enabled-skill snapshot

From `vigil.yml`, extract for each skill under `skills:`:
- `enabled` (true/false)
...
For each custom-skill candidate: confirm it exists on disk (`skills/${name}/SKILL.md`) and is **not** present in the upstream-fingerprint heuristic (skills with `install: ./add-skill besley1600/vigil ${name}` in this fork's `skills.json` are upstream; everything else is custom). Cross-reference custom skills against the Review patterns — a custom skill that uses `chains:` consume is the highest-priority audit candidate.
...
| Tag | Heuristic |
|-----|-----------|
| `trivial` | Config rename only; one-line `vigil.yml` edit |
...
*Audit basis: vigil.yml + skills.json + MEMORY.md + skills/ on disk · Manifest version: embedded in skills/v4-readiness/SKILL.md as of ${today}*
...
- This skill never modifies `vigil.yml`, never opens a PR, never pulls upstream. It only reports.
...
| `V4_READINESS_NO_CONFIG` | `vigil.yml` unreadable; fork not initialized | No |
...
- **Never auto-mutate the fork.** The skill is read-only. It does not edit `vigil.yml`, does not open a PR, does not pull upstream. The upgrade decision belongs to the operator.
...
- **Empty `vigil.yml` skills block (fresh fork)** — verdict is `READY`, every bucket is empty. Article writes, notification fires with the single-line body. The operator confirms the fork has nothing to migrate.
