...
Today is ${today}. Create a new Vigil instance by forking this repo, configuring it for a specific purpose, validating the configuration, and registering it in the fleet.
...
```json
{
  "instances": [
    {
      "name": "crypto-tracker",
      "repo": "OWNER/vigil-crypto-tracker",
...
cd "$TMPDIR/repo"
git config user.name "vigilframework"
git config user.email "vigilframework@proton.me"
```

#### 6a. Write a customized `vigil.yml`

- Read the parent `vigil.yml` as a base (preserves comments, ordering, and the full skill list).
...
```markdown
# Vigil Instance Setup

This is a managed Vigil instance spawned from `${PARENT_UPSTREAM}`.
...
## About This Repo
- Vigil instance of ${PARENT_UPSTREAM}. Purpose: ${PURPOSE}.
...
- **Never push to the parent without committing `memory/instances.json` via the normal Vigil workflow.** The parent commit happens through the same path as any other skill output.
