...
First read `vigil.yml` and find the `gateway.provider` value. Use the matching table.
...
- **Vigil.yml mismatch**: `vigil.yml` sets a `model:` override for the skill but the CSV shows runs on a different model. → "Model override drift — vigil.yml says X, runs show Y."
...
---
*Sources: token-usage.csv ({{ok|degraded: M malformed rows skipped}}) · vigil.yml ({{ok|missing}}) · pricing table last reviewed in SKILL.md.*
*Generated by Vigil cost-report skill.*
```
...
- Source status: csv={{ok|degraded}}, vigil.yml={{ok|missing}}
...
No outbound network required — this skill only reads local files (`memory/token-usage.csv`, `vigil.yml`). If future versions pull the Anthropic Usage/Cost API, use WebFetch as the fallback for sandboxed curl, and cache results to `.xai-cache/` via a pre-fetch script (see CLAUDE.md).
