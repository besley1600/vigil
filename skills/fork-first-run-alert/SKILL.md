...
| Side | What they get |
|------|---------------|
| Operator (@besley1600 + community ops) | Same-day named ping — "Fork `speend/vigil` just ran its first skill" — with link, run count if detectable, fork stargazers |
...
Exclude bot allowlist from `NEW_ACTIVE` (same list as fork-cohort): `dependabot[bot]`, `github-actions[bot]`, `vigilframework[bot]`. Bot-owned forks still get added to `seen` so they aren't re-evaluated forever — just not alerted on.
...
- `workflow_name` — the workflow file name (e.g. `vigil.yml`)
...
@{fork.owner} just ran their first workflow on Vigil. Welcome to the running fleet.
