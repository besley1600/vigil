...
3. Emits a *delta* against current `vigil.yml` rather than a full config dump.
...
- `vigil.yml` — CURRENT skill enablement, `var`, `schedule`, `model` per skill (this is the comparison baseline)
...
### 4. Compare against current vigil.yml (delta, not dump)

For each tiered recommendation, compute the delta:

| Recommended state | Current state in vigil.yml | Action |
|-------------------|--------------------------|--------|
