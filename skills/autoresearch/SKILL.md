...
- Follow Vigil skill conventions (read memory, log to memory/logs/${today}.md, notify via `./notify`)
...
| **Conventions** | Does it follow Vigil patterns? (memory, logging, notify, var usage) |
...
- Do not add env vars that aren't already available in the workflow (check vigil.yml secrets).
