...
Read this running instance's local `vigil.yml` once. Build:
...
Then fetch the fork's `vigil.yml` only if the tree contains it:

```bash
gh api "repos/${FORK_FULL}/contents/vigil.yml?ref=${FORK_DEFAULT_BRANCH}" --jq '.content' | base64 -d
```

Error handling:
- 404 / 409 (empty repo): mark `status: "no_tree"`, skip vigil.yml extraction, continue.
- 403 with `X-RateLimit-Remaining: 0`: sleep 60s, retry once. If still failing, mark `status: "rate_limited"` and continue.
- Tree contains vigil.yml but contents call 404s: mark `status: "yml_unreadable"`, continue.
- vigil.yml present but YAML parse fails: mark `status: "yml_invalid"`, continue.

For each readable `vigil.yml`, extract per-skill `{enabled, model, var, schedule}`. Treat missing keys as inheriting the upstream default (do NOT count those as overrides).
...
- **TEMPLATE**: vigil.yml readable but every signal is 0 — exclude from divergence math
- **UNREADABLE**: no_tree / no vigil.yml / yml_unreadable / yml_invalid / rate_limited — tracked in source-status footer
...
Rank forks by `total_overrides` desc. Top 5 = "heaviest customizers" — surface in the article with their dominant category and a one-line synthesis (e.g., `"owner/vigil — content-heavy: 14 article/digest skills enabled, 3 model overrides to claude-sonnet-4-6"`).
...
*Scanned ${N_ACTIVE} active forks of ${TARGET_REPO} (pushed in last 30 days). ${N_CONFIGURED} are configured (vigil.yml diverges from upstream defaults). Divergence scored against the configured ${N_CONFIGURED}.*
...
| Template (untouched vigil.yml) | N_TEMPLATE | XX% |
...
- vigil.yml readable: (N_CONFIGURED + N_TEMPLATE) / N_ACTIVE
...
---
*Source: GitHub API — forks of ${TARGET_REPO}. Methodology: a fork is "configured" if its vigil.yml diverges from upstream defaults on enabled, model, var, or schedule for any skill. Untouched templates are excluded from divergence math. Companion to skill-leaderboard (popularity) and fork-fleet (per-fork work).*
```
