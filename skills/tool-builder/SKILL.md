...
Your job is to ship one small, self-contained CLI tool that future Vigil runs (and the operator) will actually re-use. The bar for "shippable" is the **Quality gates** in step 3 — a tool that doesn't pass them is not done.
