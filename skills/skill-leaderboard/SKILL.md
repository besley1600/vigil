...
Today is ${today}. Generate a weekly leaderboard of which Vigil skills the **configured fleet** is actually running — and what upstream should do about it.
...
Handle errors:
- 404 / 409 (empty repo): mark `status: "no_tree"`, skip vigil.yml and skills/ extraction, continue.
...
```bash
gh api "repos/${FORK_FULL}/contents/vigil.yml" --jq '.content' | base64 -d
```

If the path is in the tree but the contents call 404s, mark `status: "yml_unreadable"` and continue.

Extract from each readable `vigil.yml`:
...
- **TEMPLATE**: vigil.yml is readable but every diff signal is 0. Untouched template — exclude from leaderboard math.
- **UNREADABLE**: no_tree / no vigil.yml / yml_unreadable / rate_limited. Tracked in the source-status footer.
...
*Scanned ${N_ACTIVE} active forks of ${TARGET_REPO} (pushed in last 30 days). ${N_CONFIGURED} are configured (vigil.yml diverges from upstream defaults). Leaderboard scored against the configured ${N_CONFIGURED}.*
...
---
*Source: GitHub API — forks of ${TARGET_REPO}. Methodology: a fork counts as "configured" if its `vigil.yml` differs from upstream defaults on `enabled`, `model`, `var`, or `schedule` for any skill. Untouched templates are excluded from leaderboard math.*
```
...
3. Else if a Fleet-only skill exists: `"${fork_owner}/vigil shipped ${skill} — not in upstream"`
