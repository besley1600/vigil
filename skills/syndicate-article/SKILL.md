...
Cross-post Vigil articles to [Dev.to](https://dev.to) (developer audience) and [Farcaster](https://warpcast.com) (crypto-native audience) for organic discovery. Articles are published with a canonical URL pointing back to the GitHub Pages gallery, preserving SEO attribution.
...
2. Rewrite relative links/images: any `](/foo)` or `](foo.md)` → absolute `https://besley1600.github.io/vigil/foo` (strip `.md` where present). Preserve anchor fragments.
...
```
https://besley1600.github.io/vigil/articles/YYYY/MM/DD/<slug>/
```
