...
c. **Regression hunter**: if `last_success` exists, run
   ...
   ```bash
   git log --oneline --since="$LAST_SUCCESS" -- skills/{name}/SKILL.md vigil.yml scripts/
   ```
...
d. **Recent failed runs (last 5, not just 1)**:
   ...
   ```bash
   gh run list --workflow=vigil.yml --limit 50 --json databaseId,name,conclusion,createdAt \
...
| **`rate-limit`** | Add backoff (`sleep`), reduce request count, or add a fallback endpoint. Never raise the limit from the skill side. If the skill's `schedule` is too aggressive, propose a less-frequent cron in the PR body but **don't edit `vigil.yml`** unless the issue file already authorizes it. |
...
| **`missing-secret`** | **Do not modify `vigil.yml` or the workflow.** File or update the issue with `status: open`, `category: missing-secret`, naming the secret. Notify operator with the env-var name. Exit `REPAIR_DIAGNOSED_NO_FIX`. |
| **`config`** | Reversible vigil.yml edits only — `schedule`, `var`, `model`, `enabled: false`. **Never** add or remove top-level structure or chains. Keep diff < 5 lines in vigil.yml. |
...
- **HIGH** — touches `vigil.yml`, removes existing features, disables a skill, modifies a `scripts/*.sh` file. **HIGH risk PRs must add the label `manual-review` and must NOT be auto-mergeable** (skip `auto-merge`-friendly framing in the PR body).
...
**Manual trigger:** [Run skill](https://github.com/{owner}/{repo}/actions/workflows/vigil.yml) with `skill={name}` and `var={var}`.
...
git add skills/{name}/SKILL.md  # plus vigil.yml or scripts/* iff in playbook
...
- Never modify secrets, the workflow file (`.github/workflows/vigil.yml`), or `messages.yml`.
- Never push to `main`. Always branch + PR.
- Never auto-merge HIGH-risk PRs. They carry the `manual-review` label.
- If a skill has been failing > 7 days with no clear root cause and the category is `unknown`, recommend (in the issue and notify) `enabled: false` in `vigil.yml` — but **do not apply that change** without an explicit operator-approved issue.
