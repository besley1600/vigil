...
Today is ${today}. Your task is to find an external skill that fills a **real** gap in this repo, install it via `./add-skill` (so `skills.lock` + `vigil.yml` + the trust-gated security scan all fire), and notify **only** when something was installed or surfaces as a strong recommendation. Silence on no-gap / empty-result runs is correct — it prevents training the operator to ignore this channel.
...
b. **Curated indexes** via `./add-skill <repo> --list` (iterate in this order):
   ...
   - `./add-skill besley1600/vigil --list` (this repo's inventory — informational, cannot re-install; any hit here signals a duplicate and confirms gap fit is probably wrong)
...
- **Gate 2 — runtime compatible.** Runs with what we have: `gh` / `curl` / `WebFetch` / `jq` / stdlib. Needs only env vars already referenced in `vigil.yml` (do not recommend skills that require `docker`, `kubectl`, a paid-only API, or secrets we can't set). When in doubt, WebFetch the SKILL.md to confirm.
...
- `vigil.yml` scheduling entry (appended disabled, operator flips `enabled: true` when ready)
...
Commit `skills/<name>/`, `skills.lock`, and `vigil.yml` changes on a branch `search-skill/<name>` and open a PR rather than pushing to main (per CLAUDE.md). The PR body should quote the candidate's description, the gap it fills, and the scores.
...
Next: review skills/<skill-name>/SKILL.md and flip vigil.yml `enabled: true` when ready.
