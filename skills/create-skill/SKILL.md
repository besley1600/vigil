...
5. **Design the skill.** Decide:
   ...
   - **Schedule suggestion** — choose a cron slot. Read existing schedules in `vigil.yml`; avoid co-scheduling at the same minute as heavy skills (article, repo-scanner, deep-research, telegram-digest) unless the new skill is lightweight (<30s expected). Prefer a `:30` minute offset if the natural hour is already crowded.
...
9. **Register in `vigil.yml`.** Insert the new skill in the appropriate time-slot section:
...
10. **Open as a PR (never commit to `main`).**
    ...
    git add skills/$name/SKILL.md vigil.yml
...
11. **Log.** Append to `memory/logs/${today}.md`:
    ...
    - Registered in vigil.yml: schedule={cron}, model={model}
...
| `CREATE_SKILL_VALIDATION_FAILED` | Quality enforcement or post-write checks failed | Delete partial files; revert vigil.yml; notify with failed criteria; stop |
...
- **Never** enable a generated skill in `vigil.yml` (`enabled: false` always — operator decides).
