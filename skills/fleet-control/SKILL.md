...
2. **Check skill is enabled in child's vigil.yml** (best-effort warning, not a block — workflow_dispatch can override `enabled: false`):
   ```bash
   gh api "repos/${REPO}/contents/vigil.yml" --jq '.content' 2>/dev/null | base64 -d \
...
3. **Trigger the skill**:
   ...
   if [ -n "$DISPATCH_VAR" ]; then
     gh workflow run vigil.yml --repo "${REPO}" -f skill="${SKILL}" -f var="${DISPATCH_VAR}" \
       && OUTCOME="dispatched" || OUTCOME="api_failed:$?"
   else
     gh workflow run vigil.yml --repo "${REPO}" -f skill="${SKILL}" \
       && OUTCOME="dispatched" || OUTCOME="api_failed:$?"
   fi
   ```
...
Failed: <name>: <reason>, ...
[If not_enabled_warn]:
Warning: <name> has skill disabled in vigil.yml — dispatched anyway
```
...
- `vigil.yml` (parse enabled skills)
