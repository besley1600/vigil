...
Read `vigil.yml` for enabled skills with schedules. Cross-reference with `memory/cron-state.json`:
...
- `vigil.yml` — enabled skill list with schedules
...
|-------|-------|-----|-----------|--------------|-----|
| VIGIL | $0.0000032626 | -11.16% | $223.4K | $41.3K | $326.3K |
...
### Rules
- Include **all** enabled skills from `vigil.yml` (not only those with recent runs). For skills with no entry in cron-state.json, show `—` for timestamp and `not yet run` in status.
...
- Never expose values from `.env`, secrets, or anything outside cron-state.json + issues/INDEX.md + vigil.yml + articles/token-report-*.md. This file is public.
