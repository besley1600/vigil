...
ZIZMOR_VERSION=$(gh api repos/woodruffw/zizmor/releases/latest --jq '.tag_name' | sed 's/^v//')
[ -z "$ZIZMOR_VERSION" ] && ZIZMOR_VERSION="1.24.1"  # fallback if gh api fails
...
${fingerprint_2} severity=High status=manual rule=unpinned-uses file=.github/workflows/vigil.yml step=Checkout
...
-->
```
...
- No new env vars beyond `GITHUB_TOKEN` / `GH_GLOBAL` (already in `vigil.yml`).
