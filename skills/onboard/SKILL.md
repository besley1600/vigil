...
1. **Manual dispatch (primary).** The operator just forked Vigil, set their secrets, and wants to confirm the agent is alive. They run `./onboard --remote` locally (or hit *Run workflow* in Actions) → this skill fires → checklist arrives in Telegram/Discord/Slack/Email.
...
```json
{
  ...
  "checks": [
    { "status": "pass", "check": "workflow .github/workflows/vigil.yml", "detail": "present", "fix": "" },
...
- **`${verdict_one_liner}`** — one of:
  - `summary.fail == 0 && summary.warn == 0` → `"All set — Vigil will run on its next cron tick."`
  - `summary.fail == 0 && summary.warn > 0` → `"Vigil will run, but {N} optional piece(s) missing."`
  - `summary.fail > 0` → `"Setup incomplete — {N} required item(s) need attention before Vigil can run."`
...
- **`${next_action}`** — derived from the highest-priority gap:
  ...
  - only `warn` → `"Optional improvements — Vigil will run regardless. Rerun ./onboard once addressed."`
  - all clean → `"Nothing — you're done. Trigger your first skill: gh workflow run vigil.yml -f skill=heartbeat"`
