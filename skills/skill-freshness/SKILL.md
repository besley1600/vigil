...
tags: [meta, dev]
---
> **${var}** — Optional. Pass `dry-run` to skip the notification (article still writes, log still appends). Pass a single skill name to scope the audit to that one consumer (e.g. `var=tweet-allocator`). Empty = audit every enabled skill in `vigil.yml`.

Today is ${today}. Walk every enabled skill in `vigil.yml`, parse the file dependencies it declares (explicit `chains: consume:` edges + implicit `articles/`, `.outputs/`, `memory/topics/`, `memory/state/` references inside each `SKILL.md`), check the on-disk freshness of each dependency against a per-class threshold, and surface a single decision-ready report: which enabled consumer is about to read a file that's older than its expected freshness window.
...
Reads:
- `vigil.yml` — enabled skill list, `chains:` blocks (steps, consume, parallel), per-skill `schedule` (used to derive expected freshness windows).
- Every `skills/*/SKILL.md` whose corresponding `vigil.yml` entry has `enabled: true` — for implicit file-reference extraction.
...
Per-class thresholds are computed at runtime — not hardcoded per dependency. The skill discovers the producer's schedule from `vigil.yml` and picks the daily-vs-weekly bucket automatically.
...
- If the remaining var is a non-empty string that matches an `vigil.yml` skill key → `SCOPE=single`, `SCOPED_SKILL=$var`. If it doesn't match any key, log `SKILL_FRESHNESS_NO_MATCH: ${var} not in vigil.yml` and exit (no notify, no article).
- Otherwise `SCOPE=fleet` and audit every enabled skill.

### 2. Load enabled-skill list and build the producer index

Parse `vigil.yml`. Build two maps:
...
Walk `vigil.yml` `chains:` blocks. For each step with a `consume: [...]` list, the consuming skill depends on `.outputs/{producer}.md` for each named producer. Record these as **explicit** edges with class `outputs` (4h threshold).
...
- `vigil.yml`: ${parsed_skill_count} entries, ${enabled_count} enabled
...
| `SKILL_FRESHNESS_NO_MATCH` | `${var}` named a skill not in vigil.yml | No |
...
- **Per-class thresholds, not per-skill.** The threshold for `articles/token-report-*.md` is the same as for `articles/repo-pulse-*.md`: the path class drives the window, derived from the producer's cadence in `vigil.yml`. This keeps the table maintainable as the fleet grows.
