...
validate_skill() {
  ...
  if [[ -n "$secret_refs" ]]; then
    while IFS= read -r ref; do
      ...
      if [[ "$known" == "false" ]]; then
        warn "references unknown variable '\$$ref' — verify it's in vigil.yml env or a known secret"
      fi
    done <<< "$secret_refs"
  fi

  # 7. Validate cron expression if skill is registered in vigil.yml with a schedule
  if [[ -f "$VIGIL_YML" ]]; then
    local cron_expr
    cron_expr=$(grep -A3 "^\s*${skill_name}:" "$VIGIL_YML" | grep "schedule:" | head -1 | sed 's/.*schedule:[[:space:]]*//' | tr -d '"' | tr -d "'" || true)
    if [[ -n "$cron_expr" ]]; then
      # Basic cron validation: must have exactly 5 space-separated fields
      local field_count
      field_count=$(echo "$cron_expr" | awk '{print NF}')
      if [[ "$field_count" -ne 5 ]]; then
        fail "invalid cron expression in vigil.yml for $skill_name: '$cron_expr' (expected 5 fields, got $field_count)"
        skill_ok=false
      else
...
echo -e "${BOLD}Vigil Skill Smoke Tests${NC}"
echo "======================"
echo ""
