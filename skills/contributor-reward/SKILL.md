...
Next: ./vigil distribute-tokens "dry-run:contributors-${TARGET_WEEK}" (preview)
      ./vigil distribute-tokens "contributors-${TARGET_WEEK}"           (execute)
```
...
- Wire as a chain (`fork-contributor-leaderboard → contributor-reward → distribute-tokens dry-run`) once the operator is comfortable with end-to-end automation. The pieces exist; the chain wiring is a one-line `vigil.yml` change.
