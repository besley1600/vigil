...
3. **Fetch all upstream PRs** in one paginated call. Keep `author_association` so we can flag first-time contributors without an extra call:
   ...
   Skip bots: any login ending in `[bot]`, plus `besley1600`, `vigilframework`, `github-actions`.
...
- Bot accounts (`*[bot]`, `github-actions`) and the core team (`besley1600`, `vigilframework`) are filtered out so the leaderboard surfaces **community** contributors only.
